...
const userId = computed(() => userStore.userInfo.userId || '教师')
const displayName = computed(() => /^demo/.test(userId.value) ? '演示教师' : userId.value)
...
const stats = [
  { label: '智课总数', value: '3', unit: '门' },
  { label: '服务学生', value: '126', unit: '人' },
  { label: '本月新增', value: '2', unit: '门' },
]
...
const menuGroups = [
  {
    title: '课程管理',
    items: [
      { icon: 'chart-trending-o', label: '学生数据',    desc: '查看学生学习统计与归因分析', path: '/m/teacher/student-data' },
      { icon: 'award-o',          label: '学生排行榜',  desc: '查看课程积分排名与进步情况', path: '/m/leaderboard' },
    ],
  },
  {
    title: '教学工具',
    items: [
      { icon: 'gem-o',            label: 'AI 助教',     desc: '智能出题、学情分析与教学优化', path: '/m/teacher/assistant' },
      { icon: 'orders-o',         label: '课程资源管理',desc: '管理课件、讲义与补充资料',     path: '/m/teacher/resources' },
    ],
  },
]
</script>
    ...
    <div class="user-card">
      ...
      <div class="user-card__inner">
        <div class="user-avatar">
          ...
          <van-icon v-else name="manager-o" size="28" color="#7c3aed" />
        </div>
        <div class="user-info">
          <h3>{{ displayName }}</h3>
      ...
      <div class="menu-list">
        ...
        >
          <div class="menu-item__icon">
            <van-icon :name="item.icon" size="20" color="#7c3aed" />
          </div>
