const DEFAULT_STATIC_KEY = import.meta.env.VITE_STATIC_KEY || 'chaoxing_default_static_key'
export const DEFAULT_SCHOOL_ID = import.meta.env.VITE_DEFAULT_SCHOOL_ID || 'sch10001'

const isFormData = (value) => typeof FormData !== 'undefined' && value instanceof FormData
const isBlobLike = (value) => typeof Blob !== 'undefined' && value instanceof Blob

const formatPart = (value) => String(value).padStart(2, '0')

export const formatSignatureTime = (date = new Date()) => (
  `${date.getFullYear()}-${formatPart(date.getMonth() + 1)}-${formatPart(date.getDate())} `
  + `${formatPart(date.getHours())}:${formatPart(date.getMinutes())}:${formatPart(date.getSeconds())}`
)

export const normalizeSignatureValue = (value) => {
  if (value === undefined || value === null) {
    return ''
  }
  if (typeof value === 'boolean') {
    return value ? 'true' : 'false'
  }
  if (Array.isArray(value) || Object.prototype.toString.call(value) === '[object Object]') {
    return JSON.stringify(value)
  }
  return String(value)
}

const collectObjectEntries = (payload = {}) => Object.entries(payload)
  .filter(([key]) => key !== 'enc' && key !== 'time')
  .filter(([, value]) => !isBlobLike(value))
  .map(([key, value]) => [key, normalizeSignatureValue(value)])
  .filter(([, value]) => value)

const collectFormEntries = (payload) => {
  const entries = []
  for (const [key, value] of payload.entries()) {
    if (key === 'enc' || key === 'time' || isBlobLike(value)) {
      continue
    }
    const normalized = normalizeSignatureValue(value)
    if (normalized) {
      entries.push([key, normalized])
    }
  }
  return entries
}

const buildSigningString = (entries, time, staticKey = DEFAULT_STATIC_KEY) => {
  const body = [...entries]
    .sort(([left], [right]) => left.localeCompare(right))
    .map(([key, value]) => `${key}${value}`)
    .join('')
  return `${body}${staticKey}${time}`
}

const toUtf8Bytes = (value) => {
  if (typeof TextEncoder !== 'undefined') {
    return Array.from(new TextEncoder().encode(value))
  }
  return Array.from(unescape(encodeURIComponent(value)), (char) => char.charCodeAt(0))
}

const safeAdd = (x, y) => {
  const lsw = (x & 0xffff) + (y & 0xffff)
  const msw = (x >> 16) + (y >> 16) + (lsw >> 16)
  return (msw << 16) | (lsw & 0xffff)
}

const bitRotateLeft = (num, cnt) => (num << cnt) | (num >>> (32 - cnt))

const md5cmn = (q, a, b, x, s, t) => safeAdd(bitRotateLeft(safeAdd(safeAdd(a, q), safeAdd(x, t)), s), b)
const md5ff = (a, b, c, d, x, s, t) => md5cmn((b & c) | ((~b) & d), a, b, x, s, t)
const md5gg = (a, b, c, d, x, s, t) => md5cmn((b & d) | (c & (~d)), a, b, x, s, t)
const md5hh = (a, b, c, d, x, s, t) => md5cmn(b ^ c ^ d, a, b, x, s, t)
const md5ii = (a, b, c, d, x, s, t) => md5cmn(c ^ (b | (~d)), a, b, x, s, t)

const binlMd5 = (words, bitLength) => {
  words[bitLength >> 5] |= 0x80 << (bitLength % 32)
  words[(((bitLength + 64) >>> 9) << 4) + 14] = bitLength

  let a = 1732584193
  let b = -271733879
  let c = -1732584194
  let d = 271733878

  for (let i = 0; i < words.length; i += 16) {
    const oldA = a
    const oldB = b
    const oldC = c
    const oldD = d

    a = md5ff(a, b, c, d, words[i], 7, -680876936)
    d = md5ff(d, a, b, c, words[i + 1], 12, -389564586)
    c = md5ff(c, d, a, b, words[i + 2], 17, 606105819)
    b = md5ff(b, c, d, a, words[i + 3], 22, -1044525330)
    a = md5ff(a, b, c, d, words[i + 4], 7, -176418897)
    d = md5ff(d, a, b, c, words[i + 5], 12, 1200080426)
    c = md5ff(c, d, a, b, words[i + 6], 17, -1473231341)
    b = md5ff(b, c, d, a, words[i + 7], 22, -45705983)
    a = md5ff(a, b, c, d, words[i + 8], 7, 1770035416)
    d = md5ff(d, a, b, c, words[i + 9], 12, -1958414417)
    c = md5ff(c, d, a, b, words[i + 10], 17, -42063)
    b = md5ff(b, c, d, a, words[i + 11], 22, -1990404162)
    a = md5ff(a, b, c, d, words[i + 12], 7, 1804603682)
    d = md5ff(d, a, b, c, words[i + 13], 12, -40341101)
    c = md5ff(c, d, a, b, words[i + 14], 17, -1502002290)
    b = md5ff(b, c, d, a, words[i + 15], 22, 1236535329)

    a = md5gg(a, b, c, d, words[i + 1], 5, -165796510)
    d = md5gg(d, a, b, c, words[i + 6], 9, -1069501632)
    c = md5gg(c, d, a, b, words[i + 11], 14, 643717713)
    b = md5gg(b, c, d, a, words[i], 20, -373897302)
    a = md5gg(a, b, c, d, words[i + 5], 5, -701558691)
    d = md5gg(d, a, b, c, words[i + 10], 9, 38016083)
    c = md5gg(c, d, a, b, words[i + 15], 14, -660478335)
    b = md5gg(b, c, d, a, words[i + 4], 20, -405537848)
    a = md5gg(a, b, c, d, words[i + 9], 5, 568446438)
    d = md5gg(d, a, b, c, words[i + 14], 9, -1019803690)
    c = md5gg(c, d, a, b, words[i + 3], 14, -187363961)
    b = md5gg(b, c, d, a, words[i + 8], 20, 1163531501)
    a = md5gg(a, b, c, d, words[i + 13], 5, -1444681467)
    d = md5gg(d, a, b, c, words[i + 2], 9, -51403784)
    c = md5gg(c, d, a, b, words[i + 7], 14, 1735328473)
    b = md5gg(b, c, d, a, words[i + 12], 20, -1926607734)

    a = md5hh(a, b, c, d, words[i + 5], 4, -378558)
    d = md5hh(d, a, b, c, words[i + 8], 11, -2022574463)
    c = md5hh(c, d, a, b, words[i + 11], 16, 1839030562)
    b = md5hh(b, c, d, a, words[i + 14], 23, -35309556)
    a = md5hh(a, b, c, d, words[i + 1], 4, -1530992060)
    d = md5hh(d, a, b, c, words[i + 4], 11, 1272893353)
    c = md5hh(c, d, a, b, words[i + 7], 16, -155497632)
    b = md5hh(b, c, d, a, words[i + 10], 23, -1094730640)
    a = md5hh(a, b, c, d, words[i + 13], 4, 681279174)
    d = md5hh(d, a, b, c, words[i], 11, -358537222)
    c = md5hh(c, d, a, b, words[i + 3], 16, -722521979)
    b = md5hh(b, c, d, a, words[i + 6], 23, 76029189)
    a = md5hh(a, b, c, d, words[i + 9], 4, -640364487)
    d = md5hh(d, a, b, c, words[i + 12], 11, -421815835)
    c = md5hh(c, d, a, b, words[i + 15], 16, 530742520)
    b = md5hh(b, c, d, a, words[i + 2], 23, -995338651)

    a = md5ii(a, b, c, d, words[i], 6, -198630844)
    d = md5ii(d, a, b, c, words[i + 7], 10, 1126891415)
    c = md5ii(c, d, a, b, words[i + 14], 15, -1416354905)
    b = md5ii(b, c, d, a, words[i + 5], 21, -57434055)
    a = md5ii(a, b, c, d, words[i + 12], 6, 1700485571)
    d = md5ii(d, a, b, c, words[i + 3], 10, -1894986606)
    c = md5ii(c, d, a, b, words[i + 10], 15, -1051523)
    b = md5ii(b, c, d, a, words[i + 1], 21, -2054922799)
    a = md5ii(a, b, c, d, words[i + 8], 6, 1873313359)
    d = md5ii(d, a, b, c, words[i + 15], 10, -30611744)
    c = md5ii(c, d, a, b, words[i + 6], 15, -1560198380)
    b = md5ii(b, c, d, a, words[i + 13], 21, 1309151649)
    a = md5ii(a, b, c, d, words[i + 4], 6, -145523070)
    d = md5ii(d, a, b, c, words[i + 11], 10, -1120210379)
    c = md5ii(c, d, a, b, words[i + 2], 15, 718787259)
    b = md5ii(b, c, d, a, words[i + 9], 21, -343485551)

    a = safeAdd(a, oldA)
    b = safeAdd(b, oldB)
    c = safeAdd(c, oldC)
    d = safeAdd(d, oldD)
  }

  return [a, b, c, d]
}

const bytesToWords = (bytes) => {
  const words = []
  for (let i = 0; i < bytes.length * 8; i += 8) {
    words[i >> 5] |= (bytes[i / 8] & 0xff) << (i % 32)
  }
  return words
}

const wordsToHex = (words) => {
  let output = ''
  for (let i = 0; i < words.length * 4; i += 1) {
    const value = (words[i >> 2] >> ((i % 4) * 8)) & 0xff
    output += value.toString(16).padStart(2, '0')
  }
  return output
}

export const md5 = (value) => {
  const bytes = toUtf8Bytes(value)
  return wordsToHex(binlMd5(bytesToWords(bytes), bytes.length * 8)).toUpperCase()
}

export const signPayloadObject = (payload = {}, staticKey = DEFAULT_STATIC_KEY) => {
  const nextPayload = { ...payload }
  const time = nextPayload.time || formatSignatureTime()
  nextPayload.time = time
  nextPayload.enc = md5(buildSigningString(collectObjectEntries(nextPayload), time, staticKey))
  return nextPayload
}

export const signPayloadFormData = (payload, staticKey = DEFAULT_STATIC_KEY) => {
  const time = payload.get('time') || formatSignatureTime()
  payload.set('time', time)
  payload.set('enc', md5(buildSigningString(collectFormEntries(payload), time, staticKey)))
  return payload
}

export const signRequestConfig = (config = {}) => {
  const method = String(config.method || 'get').toLowerCase()
  if (method === 'get' && config.params && !config.skipSignature) {
    config.params = signPayloadObject(config.params)
    return config
  }

  if (config.skipSignature || !config.data) {
    return config
  }

  if (isFormData(config.data)) {
    config.data = signPayloadFormData(config.data)
    return config
  }

  if (Object.prototype.toString.call(config.data) === '[object Object]') {
    config.data = signPayloadObject(config.data)
  }
  return config
}

export const buildSignedPath = (path, params = {}, staticKey = DEFAULT_STATIC_KEY) => {
  const signed = signPayloadObject(params, staticKey)
  const search = new URLSearchParams()
  Object.entries(signed).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== '') {
      search.set(key, String(value))
    }
  })
  const query = search.toString()
  return query ? `${path}?${query}` : path
}
