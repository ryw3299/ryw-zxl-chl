...
Before any PPT generation task, determine the execution mode:

- **Courseware / teaching context** — if the user asks for "课件", "教案", "讲课", "课程", "教学", "课堂", "courseware", "teaching slides", "lecture", or "lesson", read [`skills/courseware-flash/SKILL.md`](skills/courseware-flash/SKILL.md) — the fast lane for teacher courseware generation.
- **General presentation context** — otherwise, read [`skills/ppt-master/SKILL.md`](skills/ppt-master/SKILL.md) — the authoritative workflow for project creation, role switching, serial execution, quality gates, post-processing, and export.

Both modes reuse the same export engine (`svg_quality_checker.py` → `total_md_split.py` → `finalize_svg.py` → `svg_to_pptx.py`).
...
- Choose the correct skill before starting: courseware/teaching requests → [`skills/courseware-flash/SKILL.md`](skills/courseware-flash/SKILL.md); general presentations → [`skills/ppt-master/SKILL.md`](skills/ppt-master/SKILL.md).
...
- On conflict with a generic coding skill, prioritize the skill chosen for the current task ([`skills/courseware-flash/SKILL.md`](skills/courseware-flash/SKILL.md) or [`skills/ppt-master/SKILL.md`](skills/ppt-master/SKILL.md)) and this file inside this repository.
...
```bash
# Courseware Flash (fast teaching deck)
python skills/courseware-flash/scripts/auto_source_convert.py <source_file> <project_path>  # optional: auto-detect & convert
...
python skills/courseware-flash/scripts/generate_spec_lock.py <project_path>   # optional
