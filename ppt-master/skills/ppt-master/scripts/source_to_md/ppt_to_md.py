...
def extract_slide_title(blocks: list[str]) -> str:
    """Infer a human-meaningful slide title from extracted text blocks."""
    for block in blocks:
...
def convert_presentation_to_markdown(
    ...
    for slide_index, slide in enumerate(presentation.slides, 1):
        ...
        slide_title = extract_slide_title(blocks)
        lines.append(f"<!-- Page {slide_index} -->")
        if slide_title:
            lines.append(f"## Slide {slide_index}: {slide_title}")
        else:
